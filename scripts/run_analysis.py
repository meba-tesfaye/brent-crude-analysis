...
def execute_bayesian_break_search():
    ...
    # Check for dataset; fallback to simulated data if missing
    if os.path.exists(raw_path):
        df = load_and_clean_data(raw_path)
    else:
        print("[Pipeline Note] data/raw_brent_prices.csv not found. Using simulated structural break data.")
        np.random.seed(42)
    ...
    with pm.Model() as cp_model:
        # Prior for when the structural break occurs
        tau = pm.DiscreteUniform("tau", lower=0, upper=len(t_arr) - 1)
        
        # Priors for the mean price before and after the break
        ...
        # Switch behavior based on tau index location
        mu_assigned = pm.math.switch(tau > t_arr, mu_1, mu_2)
        y_obs = pm.Normal("y_obs", mu=mu_assigned, sigma=sigma, observed=y_arr)
        
        print("[MCMC Sampler] Drawing samples (Optimized for speed)...")
        # Kept draws tight to ensure swift execution
        trace = pm.sample(draws=200, tune=100, chains=1, random_seed=42, return_inferencedata=True, progressbar=True)
    
    # Gather model conclusions
    summary = az.summary(trace, var_names=["mu_1", "mu_2", "tau"])
    print("\n=== Model Parameters Summary ===")
    print(summary)
    
    # Calculate exact break properties safely using Pandas mode extraction
    ...
    # Export payload to JSON for backend integration
    os.makedirs("data", exist_ok=True)
    with open("data/model_output.json", "w") as f:
        json.dump(output_payload, f, indent=4)
        
    print(f"\n[Success] Pipeline run complete! Data successfully stored in data/model_output.json")
